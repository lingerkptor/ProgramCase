...
//                        .collect(Collectors.toList()));
     ...
     */
    private Optional<JobKey> findJobKey(String name, String... group) throws SchedulerException {
        GroupMatcher<JobKey> groupMatcher;
        switch (group.length) {
            case 1:
                groupMatcher = GroupMatcher.jobGroupEquals(group[0]);
                break;
            default:
                groupMatcher = GroupMatcher.anyJobGroup();
                break;
        }
        return scheduler.getJobKeys(groupMatcher).stream()
    ...
    @PatchMapping("/job/pause/{group}/{name}")
    public void pauseJob(@PathVariable String group, @PathVariable String name) throws SchedulerException {
        JobKey jobKey = this.findJobKey(name, group).orElseThrow();
        scheduler.pauseJob(jobKey);
    }

    @PatchMapping("/job/resume/{group}/{name}")
    public void resumeJob(@PathVariable String group, @PathVariable String name) throws SchedulerException {
        JobKey jobKey = this.findJobKey(name, group).orElseThrow();
    ...
    @DeleteMapping("/job/delete/{group}/{name}")
    public Boolean delete(@PathVariable String group, @PathVariable String name) throws SchedulerException {
        JobKey jobKey = this.findJobKey(name, group).orElseThrow();
        return scheduler.deleteJob(jobKey);
    }
    ...
    public JobDto addJob(@PathVariable String group, @PathVariable String user) {
        ...
        JobDetail jobDetail = JobBuilder.newJob(DemoJob1.class)
                ...
                .build();
        System.out.println("user: " + user + " group: " + jobDetail.getKey().getGroup());
